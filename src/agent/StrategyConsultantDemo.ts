 ...
 */
export class StrategyConsultantDemo {
  ...
  async analyzeMarketEntry(
    ...
    const analysis = await this.marketAnalysis.analyzeMarket(
      industry, 
      region, 
      mockResponse
    );
  ...
  async performCompetitiveAnalysis(
    ...
    const analysis = await this.competitiveAnalysis.analyze(
      company,
      competitors,
      mockResponse
    );
  ...
  async generateExecutiveSummary(analysis: AnalysisResult): Promise<string> {
...
*This is a demonstration analysis. For production use, connect to the Claude Code API.*`;
  ...
  private generateMockAnalysis(company: string, framework: string, depth: string): string {
...
- Focus on digital transformation and innovation`;
  ...
  private generateMockMarketAnalysis(industry: string, region: string, company?: string): string {
    return `
...
- Implement phased expansion strategy`;
  ...
  private generateMockCompetitiveAnalysis(company: string, competitors: string[]): string {
    return `
...
- Invest in digital capabilities`;
  ...
  private formatKeyFindings(analysis: AnalysisResult): string {
    if (analysis.framework === 'swot' && analysis.analysis.keyInsights) {
      return analysis.analysis.keyInsights.map((insight: string) => `- ${insight}`).join('\n');
    }
